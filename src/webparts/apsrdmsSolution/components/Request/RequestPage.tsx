...
export default class RequestPage extends React.Component<IApsrdmsSolutionProps, any> {
    private _sp: any;
    private peoplePickerContext: IPeoplePickerContext;
    // export const RequestPage: React.Component<IApsrdmsSolutionProps> = (props: IApsrdmsSolutionProps) => {
    ...
    public async onRequestInitiate(formValue: any) {
        ...
        await spCrudObj.createDocumentLibrary(libraryName, this.props).then(async (result) => {
            ...
            await this.addColumns(libraryName, this.props.context, list);
    ...
    public addColumns = async (libraryName: string, context: any, list: any) => {
        try {
            // Add custom text fields
            await list.fields.addText("FullName", { Required: false });
            

            console.log("Columns added successfully");

            // Create a custom view to display selected fields
            await list.views.add("Custom Library View", false, {
                ViewFields: [
                    "FullName"
                ],
                RowLimit: 100,
            ...
            console.error("Error adding columns or creating view:", error);
        }
    };
